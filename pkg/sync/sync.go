...
func createNetrc(workDir, token string) (string, error) {
...
func buildGitURL(org, repo, token string) string {
	// Format changed to use 'oauth2:' prefix with the token for GitHub API authentication
	return fmt.Sprintf("https://oauth2:%s@github.com/%s/%s.git", token, org, repo)
}
...
func SyncLFSContent(repoName, workDir, targetOrg, token string) error {
	...
	netrcPath, err := createNetrc(workDir, token)
	if err != nil {
		return err
	}
	defer os.Remove(netrcPath) // Clean up after operation

	// Set environment variables
	env := append(os.Environ(),
		"GIT_LFS_SKIP_SMUDGE=1",
		"GIT_TERMINAL_PROMPT=0",
		fmt.Sprintf("NETRC=%s", netrcPath),
		"GIT_TRACE=1",
		"GIT_CURL_VERBOSE=1",
	)
